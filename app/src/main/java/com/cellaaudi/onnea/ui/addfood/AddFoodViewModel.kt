        ...
        client.enqueue(object : Callback<SearchFoodResponse> {
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                    ...
                    Log.e("AddFoodViewModel", "onFailureResponse: ${response.code()} ${response.message()} ${response.raw().request.url.toString()}")
                }
            }

            override fun onFailure(call: Call<SearchFoodResponse>, t: Throwable) {
                _load.value = false
                Log.e("AddFoodViewModel", "onFailure: ${t.message.toString()}")
            }
        })
    }
}